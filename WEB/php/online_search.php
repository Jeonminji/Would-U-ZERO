...
<div id="wrap">
	...
	<div class="product">
		<?php                           
			$sql = mq("SELECT * FROM online_info WHERE $search_type LIKE '%$search%'");
			...
			//게시물 목록               
            $sql2 = mq("SELECT * FROM online_info WHERE $search_type LIKE '%$search%' LIMIT $page_start, $list");
			...
			<ul class="pagination">
				<?php
					if ($page <= 1){ } 
						...
						if(isset($unum)){
							echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=1' aria-label='Previous'>처음</a></li>";
						} else {
							echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=1' aria-label='Previous'>처음</a></li>";
						}
					}
					...
					if ($page <= 1){ } 
						...
						$pre = $page - 1;
						if(isset($unum)){
							echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=$pre'>◀&nbsp이전</a></li>";
						} else {
							echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=$pre'>◀&nbsp이전</a></li>";
						}
					}
					...
					for($i = $block_start; $i <= $block_end; $i++){
						if($page == $i){
							...
							if(isset($unum)){
								echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=$i'>$i</a></li>";
							} else {
								echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=$i'>$i</a></li>";
							}
						}
					...
					if($page >= $total_page){ } 
						...
						$next = $page + 1;
						if(isset($unum)){
							echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=$next'>다음&nbsp▶</a></li>";
						} else {
							echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=$next'>다음&nbsp▶</a></li>";
						}
					}
					...
					if($page >= $total_page){ } 
						...
						if(isset($unum)){
							echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=$total_page'>마지막</a>";
						} else {
							echo "<li class='page-item'><a class='page-link' href='online_search.php?search-type=$search_type&search=$search&page=$total_page'>마지막</a>";
						}
					}
